...
* Можно ссылаться на значения measure как `раздел переменных <ссылка>`_. Это значит, что значение может быть использована почти в любой meter или параметре measure. В качестве раздела переменных значения используются "буквально", это значит, что оно не учитывает автоматически диапазон и размеры.
* Measure может `вызвать действие <ссылка>`_, когла значение переходит в определённый диапазон.
