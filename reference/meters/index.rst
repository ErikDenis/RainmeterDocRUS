...
Формат
++++
Meter записывается как раздел в скине. Все meters имеют параметр ``Meter``, который нужет для определения всего раздела как тип meter. Большинство параметров meter зависят от его типа, но есть некоторые `общие параметры <ссылка>`_, которые действительны для некоторыз или всех meters.
